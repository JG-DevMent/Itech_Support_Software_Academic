const configRecibosModel = require('../models/configRecibosModel');

exports.obtenerConfig = async (req, res) => {
  try {
    const config = await configRecibosModel.obtener();
...
exports.actualizarConfig = async (req, res) => {
  try {
    const actualizado = await configRecibosModel.actualizar(req.params.id, req.body);
